...
public class CredValuesPage {
	...
	public Map<String, Map<String, Integer>> getAllCred() {
		...
		for (WebElement tripElement : tripList) {
			try {
				i++;
				if (i > 5)
					break; // Process only the first 5 trips
					...
					for (WebElement tripEle : tripdata) {
							...
							if (lastThreeDigit != null) {
								try {
									...
									WbidBasepage.logger.info(
											"Extracted Total Credit (Rpt) for " + tripCode + ": " + extractedValue);
									totalCredMapUI.put(tripCode, extractedValue);
								} catch (NumberFormatException e) {
									WbidBasepage.logger
											.fail("Failed to parse extracted total credit: " + lastThreeDigit);
								}
							}
						...
						if (lastThreeDigit != null) {
							try {
								int extractedValue = Integer.parseInt(lastThreeDigit);
								WbidBasepage.logger
										.info("Extracted Individual Credit for " + tripCode + ": " + extractedValue);
								credMapUI.put(tripCode, extractedValue);
							} catch (NumberFormatException e) {
								WbidBasepage.logger
										.fail("Failed to parse extracted individual credit: " + lastThreeDigit);
							}
						}
					}
				}
	...
	public Map<String, Map<String, List<Integer>>> tripDataWithLine() {
		Map<String, Map<String, List<Integer>>> allTripsMap = new HashMap<>(); // Stores (Line -> (Trip Code -> [Date,
																				// Credits]))
		Map<String, Integer> totalCredMapUI = new HashMap<>();
			...
			try {
				...
				for (WebElement tripElement : tripList) {
					try {
						...
						if (processedTrips.contains(tripText))
							continue; // Skip already processed trips
						...
						if (!tripMatcher.find())
							continue; // Skip if trip code not found
						...
						for (WebElement tripEle : tripdata) {
							...
							if (tripDataText.startsWith("Rpt")) {
								...
								String lastThreeDigit = null;
								while (rptMatcher.find())
									lastThreeDigit = rptMatcher.group(1);

								if (lastThreeDigit != null) {
									totalCredMapUI.put(tripCode, Integer.parseInt(lastThreeDigit));
									WbidBasepage.logger.info("Line No: " + i + " Extracted Total Credit (Rpt) for "
											+ tripCode + ": " + lastThreeDigit);
								}
								continue;
							}
							...
							if (!creditValues.isEmpty()) {
								...
								WbidBasepage.logger.info(
										"Extracted Credits for " + tripCode + " on " + tripDate + ": " + creditValues);
							}
						}
	...
	public Map<String, Map<String, Map<String, List<Integer>>>> getTripDataWithLine() {
		...
		for (int i = 1; i < lineCount; i++) {
			try {
				String tripXPath = getTripXPath(i);
				List<WebElement> tripList = driver.findElements(By.xpath(tripXPath));
				Map<String, Map<String, List<Integer>>> tripDataPerLine = new HashMap<>(); // Stores (Trip Code -> [Date
																							// -> Credits])
				...
				for (WebElement tripElement : tripList) {
					try {
						...
						for (WebElement tripEle : tripdata) {
							String tripDataText = objaction.gettext(tripEle).trim().replaceAll("\\s+", " ");
							// WbidBasepage.logger.info("Line No: " + i + " Trip Data Text: " +
							// tripDataText);
							...
							String tripDate = dateMatcher.group(1).trim();
							// WbidBasepage.logger.info("Line No: " + i + " Extracted Trip Code: " +
							// tripCode + " and Trip Date: " + tripDate);
							...
							if (!creditValues.isEmpty()) {
								credMapUI.putIfAbsent(tripDate, new ArrayList<>());
								credMapUI.get(tripDate).addAll(creditValues);
								// WbidBasepage.logger.info("Extracted Credits for " + tripCode + " on " +
								// tripDate + ": " + creditValues);
							}
						}
